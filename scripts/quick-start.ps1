...
function Invoke-Command-WithCheck {
    ...
    try {
        ...
        Write-Host "❌ $Description failed with error: $_" -ForegroundColor Red
        exit 1
    }
    Write-Host ""
}
...
if (-not (Test-Path $envDir)) {
    Write-Host "❌ Environment directory not found: $envDir" -ForegroundColor Red
    exit 1
}

Push-Location $envDir
Write-Host "✅ Changed to directory: $envDir" -ForegroundColor Green
Write-Host ""
...
try {
    ...
    if (-not (Test-Path "terraform.tfvars")) {
        ...
        Write-Host "📄 terraform.tfvars already exists, skipping copy" -ForegroundColor Yellow
        Write-Host ""
    }
    ...
    $profile = if ($Environment -eq "personal") { "personal" } else { "default" }
    Write-Host "   aws ec2 describe-instances --filters 'Name=tag:kubernetes.io/cluster/team2-infra-$Environment-cluster,Values=owned' --query 'Reservations[].Instances[].InstanceId' --output table --profile $profile" -ForegroundColor Gray
    Write-Host ""
    Write-Host "4. Configure kubectl:" -ForegroundColor White
    Write-Host "   aws eks update-kubeconfig --region ap-northeast-2 --name team2-infra-$Environment-cluster --profile $profile" -ForegroundColor Gray
    Write-Host ""
